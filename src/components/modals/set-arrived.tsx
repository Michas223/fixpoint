...
import { SetOrderArrivedAtSchema } from "@/lib/zod";
import z from "zod";
...
import { setArrivedAt } from "@/actions/orders";
import {
    Popover,
    PopoverContent,
    PopoverTrigger,
...
export default function SetArrivedAtModal({
    orderId,
    setShow,
}: {
    orderId: string;
    ...
    const form = useForm<z.infer<typeof SetOrderArrivedAtSchema>>({
        resolver: zodResolver(SetOrderArrivedAtSchema),
        defaultValues: {
            arrivedAt: new Date(),
        },
    });
    ...
    const onSubmit = async (
        ...
        try {
            const res = await setArrivedAt(orderId, values);
